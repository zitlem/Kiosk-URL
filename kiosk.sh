...
CONFIG_FILE="$INSTALL_DIR/kiosk_url.txt"
API_CONFIG_FILE="$INSTALL_DIR/api_config.json"
ROTATION_CONFIG_FILE="$INSTALL_DIR/rotation_config.txt"
PLAYLIST_CONFIG_FILE="$INSTALL_DIR/playlist_config.json"
...
create_default_playlist() {
    log_info "Creating default playlist configuration..."
  ...
  "urls": [
    ...
    echo "$default_config" > "$PLAYLIST_CONFIG_FILE"
    chmod 644 "$PLAYLIST_CONFIG_FILE"
    
    log_debug "Default playlist configuration created"
}
...
get_playlist_config() {
    ...
    if validate_playlist_config "$PLAYLIST_CONFIG_FILE" >/dev/null 2>&1; then
        cat "$PLAYLIST_CONFIG_FILE"
    else
        log_warn "Invalid playlist config, using default"
        create_default_playlist
        cat "$PLAYLIST_CONFIG_FILE"
    fi
}
...
create_default_configs() {
    ...
    echo "http://example.com" > "$CONFIG_FILE"
    echo "normal" > "$ROTATION_CONFIG_FILE"
    create_default_playlist
    
    chmod 644 "$CONFIG_FILE" "$ROTATION_CONFIG_FILE" "$PLAYLIST_CONFIG_FILE"
}
...
ROTATION=\$(cat "$ROTATION_CONFIG_FILE" 2>/dev/null || echo "normal")
...
" 2>/dev/null)
    ...
    # Single URL mode
    URL=\$(cat "$CONFIG_FILE" 2>/dev/null || echo "http://example.com")
...
try:
    ...
    if current_index < len(urls):
        display_time = urls[current_index].get('display_time', data.get('default_display_time', $DEFAULT_DISPLAY_TIME))
        print(display_time)
    else:
...
PLAYLIST_CONFIG_FILE = "/opt/kiosk/playlist_config.json"
...
def load_api_config():
    try:
        ...
        return {"api_key": "default-key", "require_auth": False}
...
print_setup_info() {
    local api_key
    api_key=$(grep '"api_key"' "$API_CONFIG_FILE" | cut -d'"' -f4 2>/dev/null || echo "default-key")
...
set_url() {
    ...
    # Set new URL
    if ! echo "$url" > "$CONFIG_FILE"; then
        log_error "Failed to save URL to config file"
        exit 1
    fi
...
set_rotation() {
    ...
    if ! echo "$rotation" > "$ROTATION_CONFIG_FILE"; then
        log_error "Failed to save rotation to config file"
        exit 1
    fi
...
get_api_key() {
    if [[ -f "$API_CONFIG_FILE" ]]; then
        grep '"api_key"' "$API_CONFIG_FILE" | cut -d'"' -f4 2>/dev/null || echo "default-key"
    else
        echo "default-key"
    fi
}
